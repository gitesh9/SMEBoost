...
class OpenAIManager:
    ...
    def generate_campaign(cls, data, mongo_id):
        ...
        return {"message": "Campaign created, content is being generated", "campaign": campaign_json}

    @classmethod
    def _generate_content_async(cls, product, campaign_json, business, mongo_id):
        try:
            # 1. Instagram Posts
            insta_prompt = ContentIn.get_prompts(business, product, campaign_json)["insta_prompt"]
    ...
    def stream_full_campaign_response(cls, mongo_id):
        data = mongo.db.businesses.find_one(
            {"_id": ObjectId(mongo_id)}
        )
        business = data.get("businessDetails", {})
        product = data.get("productDetails", {})
        ...
        def stream():
            try:
                ...
                cls.attach_openai_data(mongo_id, {"openai.campaign": campaign_json, "openai.blog": blog_content,
